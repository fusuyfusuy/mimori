...
#[test]
fn test_cli_find_limit_flag() {
    let dir = tempdir().unwrap();
    let src_dir = dir.path().join("src");
    fs::create_dir_all(&src_dir).unwrap();

    let rs_file = src_dir.join("symbols.rs");
    let mut code = String::new();
    for i in 0..10 {
        code.push_str(&format!("pub fn sample_fn_{}() {{}}\n", i));
    }
    fs::write(&rs_file, &code).unwrap();
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("3 matches of 10 [--limit]"));

    // --limit 0 returns empty matches without error
    let mut cmd_zero = Command::cargo_bin("mimori").unwrap();
    cmd_zero
        .current_dir(dir.path())
        .arg("find")
        .arg("sample_fn")
